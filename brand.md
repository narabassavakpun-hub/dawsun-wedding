...
### ⚠️ ข้อยกเว้นที่จงใจ — ปิดการซูม

เว็บนี้ตั้ง `touch-action: pan-y` และ `user-scalable=no` ซึ่ง**ขัดกับแนวปฏิบัติด้านการเข้าถึงตามปกติ**

เหตุผล: พอหน้าจอถูกซูมเข้า (ผู้ใช้ถ่างนิ้ว หรือ iOS ซูมเองตอนโฟกัสช่องกรอก)
ผู้ใช้จะเลื่อนซ้ายขวาได้เสมอ เพราะเป็นการเลื่อน **visual viewport** ซึ่ง `overflow-x` ห้ามไม่ได้
อาการคือหน้าจอ "หลุด" เลื่อนไปเห็นขอบว่างๆ โดยเฉพาะหลังสลับแอปกลับมา
ซึ่งสร้างความสับสนกับแขกในงานมากกว่าประโยชน์ของการซูม

สิ่งที่ทำเพื่อชดเชย — ต้องรักษาไว้ถ้าจะแก้โค้ดต่อ:
- `--fs-body` ต่ำสุด 15px และ `--fs-body-lg` สำหรับข้อความสำคัญ
- ปุ่มทุกตัวสูง ≥ 48px
- ช่องกรอกบังคับ `font-size: max(16px, ...)` กัน iOS auto-zoom (คนละกลไกกับ pinch)

ถ้าอนาคตแก้จนไม่มีอาการจอหลุดแล้ว ควรคืน `pinch-zoom` กลับ
